...
export default async function (projectName) {
    ...
    const paths = await assemblePath({ projectName, projectRoot: "__GEN_PROJECTS", rootPath: "databases", tech: "postgresql" });

    console.log(`🚀 Creating Postgresql project: ${projectName}...`);
    ...
    try {
        ...
        console.log("ⓘ  The host url is localhost:5432");
        console.log("ⓘ  The usename is postgresqladmin");
...
function initialiseFramework({ projectName, techDir, projectDir, toolsDirTechno }) {
    console.log(`🚀 Initialising Postgresql project: ${projectName}...`);
...
function settingContainer({ projectDir }) {
    ...
    try {
        ...
        waitForContainerHealthy("multitech-postgresql-server", 60); // 60 seconds timeout
