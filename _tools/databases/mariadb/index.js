...
export default async function (projectName) {
    ...
    const paths = await assemblePath({ projectName, projectRoot: "__GEN_PROJECTS", rootPath: "databases", tech: "mariadb" });

    console.log(`🚀 Creating MariaDB project: ${projectName}...`);
    ...
    try {
        ...
        console.log("ⓘ  The host url is localhost:3306");
        console.log("ⓘ  The usename is mariadbamdin");
...
function initialiseFramework({ projectName, techDir, projectDir, toolsDirTechno }) {
    console.log(`🚀 Initialising MariaDB project: ${projectName}...`);
...
function settingContainer({ projectDir }) {
    ...
    try {
        ...
        waitForContainerHealthy("multitech-mariadb-server", 60); // 60 seconds timeout
