...
export default async function (projectName) {
    ...
    const paths = await assemblePath({ projectName, projectRoot: "__GEN_PROJECTS", rootPath: "databases", tech: "oracleenterprise" });

    console.log(`🚀 Creating Oracle Enterprise project: ${projectName}...`);
    ...
    try {
        ...
        console.log("ⓘ  The host url is localhost:1521");
        console.log("ⓘ  The usename is system");

        console.log("⚠️  The first time it start, oracle need to initialise and it can take several minutes");
...
function initialiseFramework({ projectName, techDir, projectDir, toolsDirTechno }) {
    console.log(`🚀 Initialising Oracle Enterprise project: ${projectName}...`);
...
function settingContainer({ projectDir }) {
    ...
    try {
        ...
        waitForContainerHealthy("multitech-oracleenterprise-server", 60); // 60 seconds timeout
